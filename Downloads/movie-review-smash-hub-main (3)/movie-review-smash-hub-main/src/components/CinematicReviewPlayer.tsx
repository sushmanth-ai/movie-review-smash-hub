...
export const CinematicReviewPlayer: React.FC<CinematicReviewPlayerProps> = ({
  ...
  const liveExpanded = isFs || isExpanded;
  const landscapeActive = liveExpanded || forceLandscape;
  ...
  return (
    <div className="w-full my-6 animate-fade-in">
      {isExpanded && !isFs && (
        <div className="fixed inset-0 z-[1998] bg-black/70 backdrop-blur-sm transition-opacity duration-500" />
      )}
      <div
        ref={containerRef}
        className={`relative w-full rounded-2xl overflow-hidden border border-amber-500/40 shadow-[0_20px_60px_-15px_rgba(255,140,40,0.35)] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${liveExpanded ? "fixed z-[1999] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100vw-1rem)] max-w-[calc(100vw-1rem)]" : ""}`}
        style={{
          background:
            "linear-gradient(180deg, rgba(8,4,16,1) 0%, rgba(20,10,8,1) 100%)",
          aspectRatio: "16 / 9",
          height: isFs ? "100vh" : undefined,
          maxHeight: liveExpanded && !isFs ? "calc(100vh - 2rem)" : undefined,
        }}
      >
