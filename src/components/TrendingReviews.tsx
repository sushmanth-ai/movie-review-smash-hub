...
const RankBadge: React.FC<{ rank: number }> = ({ rank }) => {
  const getBadgeStyle = () => {
    switch (rank) {
      case 1: return 'bg-gradient-to-br from-yellow-400 via-yellow-500 to-amber-600 text-yellow-950 shadow-[0_0_30px_rgba(255,215,0,0.8)] animate-pulse';
      case 2: return 'bg-gradient-to-br from-slate-300 via-slate-400 to-slate-500 text-slate-950 shadow-[0_0_25px_rgba(200,200,200,0.7)] animate-pulse';
      case 3: return 'bg-gradient-to-br from-amber-600 via-amber-700 to-amber-800 text-amber-100 shadow-[0_0_15px_rgba(180,120,60,0.5)]';
      default: return 'bg-muted text-muted-foreground';
    }
  };
  const getSizeStyle = () => {
    switch (rank) {
      case 1: return 'w-14 h-14 -top-4 -left-4 text-xl';
      case 2: return 'w-12 h-12 -top-3 -left-3 text-lg';
      default: return 'w-8 h-8 -top-2 -left-2 text-sm';
    }
  };
  return (
      ...
      <div className="flex gap-3 p-3">
        ...
        <div className="flex flex-col justify-between flex-1 min-w-0 py-1">
          ...
          <div className="flex items-center gap-3 text-xs text-muted-foreground mt-2">
            <div className="flex items-center gap-1"><Eye className="w-3 h-3" /><span>{review.weeklyViews}</span></div>
            <div className="flex items-center gap-1"><Heart className="w-3 h-3" /><span>{review.weeklyLikes}</span></div>
            <div className="flex items-center gap-1"><MessageCircle className="w-3 h-3" /><span>{review.weeklyComments}</span></div>
          </div>
        </div>
      </div>
...
const TrendingSkeleton: React.FC = () => (
  <div className="flex gap-3 p-3 bg-card/50 rounded-xl border border-border">
    ...
    <div className="flex flex-col justify-between flex-1 py-1">
      <div><Skeleton className="h-4 w-3/4 mb-2" /><Skeleton className="h-3 w-1/2" /></div>
      <Skeleton className="h-3 w-2/3" />
    </div>
  </div>
);

export const TrendingReviews: React.FC<TrendingReviewsProps> = ({ reviews, isLoading, limit }) => {
  const { t } = useLanguage();
    ...
    return (
      <section className="mb-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-primary">{t('trendingReviews')}</h2>
        </div>
    ...
    <section className="mb-12">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-primary">{t('trendingReviews')}</h2>
            <p className="text-xs text-muted-foreground mt-0.5">{t('basedOnEngagement')}</p>
          </div>
        </div>
        <div className="hidden sm:flex items-center gap-1 px-3 py-1.5 bg-primary/10 rounded-full border border-primary/20">
          <TrendingUp className="w-4 h-4 text-primary" />
          <span className="text-xs font-medium text-primary">{t('live')}</span>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
