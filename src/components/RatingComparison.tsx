...
export const RatingComparison: React.FC<RatingComparisonProps> = ({
  ...
  useEffect(() => {
    const loadAudienceRating = async () => {
      if (!db || !movieId) { setIsLoading(false); return; }
      try {
  ...
  const getInsightText = () => {
    if (audienceCount === 0) return t('beFirstToRate');
    if (absDiff < 0.3) return t('criticsAndAudienceAgree');
    if (difference > 0) return t('audienceLoveMore');
    return t('criticsCraftMore');
  };

  const getInsightIcon = () => {
    if (audienceCount === 0 || absDiff < 0.3) return <Minus className="w-4 h-4 text-blue-400" />;
    ...
    <div className="bg-gradient-to-br from-card/80 to-card/40 backdrop-blur-sm rounded-xl p-5 border-2 border-primary/30 shadow-[0_0_25px_rgba(255,215,0,0.2)]">
      <div className="flex items-center justify-center gap-3 mb-5 pb-3 border-b border-primary/20">
        ...
        <div className="text-center">
          ...
          <p className="text-[10px] text-muted-foreground uppercase tracking-widest">{t('ratingComparison')}</p>
        </div>
      </div>
